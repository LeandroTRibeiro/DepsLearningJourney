<img src="../../assets/csharp-logo.png" alt="Logo c#" style="width:100px; margin-top:20px"/>
...
- [**ReverseWordsInString**](ReverseWordsInString/ReverseWordsInString/Program.cs): uma aplicação de console em C# que inverte a ordem das palavras em uma string, mantendo a ordem dos caracteres em cada palavra. O programa possui duas abordagens: uma que utiliza métodos como `Split`, `Reverse`, e `Join`, e outra implementada manualmente usando loops. Além disso, benchmarks foram realizados utilizando [**BenchmarkDotNet**](ReverseWordsInString/ReverseWordsInStringBenchmark/Program.cs) para comparar o desempenho das duas soluções. A aplicação inclui [testes automatizados](ReverseWordsInString/ReverseWordsInStringTest/ProgramTest.cs) para validar o comportamento correto das funções, incluindo cenários com strings vazias, múltiplos espaços e strings contendo apenas uma palavra.


- [**FindLongestUniqueSubstring**](FindLongestUniqueSubstring/FindLongestUniqueSubstring/Program.cs): uma aplicação de console em C# que encontra a substring mais longa sem caracteres repetidos em uma string. O programa possui duas abordagens: a primeira, mais eficiente, utiliza um laço com verificações manuais, enquanto a segunda utiliza estruturas como `HashSet` e LINQ para manipulação. Além disso, benchmarks foram realizados utilizando [**BenchmarkDotNet**](FindLongestUniqueSubstring/FindLongestUniqueSubstringBenchmark/ProgramBenchmark.cs) para comparar o desempenho das duas soluções. A aplicação inclui [testes automatizados](FindLongestUniqueSubstring/FindLongestUniqueSubstring.Test/ProgramTest.cs) para garantir o comportamento correto das funções em diferentes cenários, como strings vazias, strings com caracteres repetidos e entradas variadas.
